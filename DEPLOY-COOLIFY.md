...
- لا توجد قاعدة بيانات ولا تخزين دائم لبيانات الدخول. Cookies الجلسة فقط تُحفظ في Docker Volume خاص كي يمكن استعادة `personal.aspx`.
...
2. المتصفح البعيد يظهر مباشرة أعلى الصفحة. اضغط **استعادة الجلسة أو تجهيز الدخول**.
3. إذا كانت الجلسة المحفوظة ما زالت صالحة ستفتح `personal.aspx` مباشرة؛ وإلا أدخل رقم الخريج والرقم القومي والكابتشا في الداشبورد.
4. اضغط **إرسال محاولة واحدة**.
5. راقب بطاقة آخر POST؛ `HTTP 200` مع رسالة الضغط يظهر كفشل دخول، وليس نجاحًا.
6. راقب اسم ومسار الصفحة الحالية أسفل المتصفح.
7. سجل التشغيل والعدادات يعرضان صفحات `.aspx` فقط، وملخص آخر POST موجود في نهاية الصفحة.

## 7. التحديث وإعادة التشغيل

- بعد Push جديد إلى Git استخدم **Redeploy** من Coolify.
- إعادة التشغيل أو Redeploy تغلق Chromium، لكن Cookies الجلسة تبقى في Volume باسم `nursing-session`. عند التشغيل التالي تُفحص على `personal.aspx` قبل طلب تسجيل دخول جديد.
- لا تنشر تحديثًا أثناء POST Pending.
- زر **جلسة جديدة** أو **مسح الحقول والجلسة** يمسح Cookies المحفوظة عمدًا.
